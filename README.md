...
|---|---|---|
| [**bKash · Nagad · Rocket · Bank — Send-Money Checkout + Auto-Verify**](./bkash-nagad-rocket-bank-payment-kit/) | A production Bangladeshi checkout copied pixel for pixel — name + phone, four method tiles, the send-money popup and its live "did the money arrive?" answer — **plus the server side**: the claim, the poll, and the webhook the SmartPay Auto Verify app posts each payment SMS to, with the matching and amount-tolerance rules (masked bKash numbers, Rocket TrxIDs, ৳1,020 on a ৳1,000 order). Money is confirmed on the server, never on Submit. Facebook-in-app-browser keyboard kit, Bangla-IME-safe inputs, refresh-proof. Runnable demo, tests | Vanilla JS (any stack) + React · Node / Laravel / WordPress server references |
...
| [**Login & Registration + Continue with Google**](./login-registration-google-auth-kit/) | The whole front door: one auth screen with sliding tabs, floating labels and dark mode, plus a Google button verified server-side. A known email signs straight in; a new one goes to registration already filled in, so no half-made accounts. The Client ID lives in an admin field with the console walkthrough beside it — the owner switches it on without touching a server | Blade + Laravel + React admin |
